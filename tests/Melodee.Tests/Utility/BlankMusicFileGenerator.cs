...
namespace Melodee.Tests.Utility
{
    ...
    public static class BlankMusicFileGenerator
    {
            ...
            MusicMetadata metadata = null)
        {
            ...
            string filePath = Path.Combine(outputPath, $"test_{versionString}_{Guid.NewGuid():N}.mp3");
        ...
        {
            byte[] bytes = Encoding.ASCII.GetBytes((text ?? "").PadRight(length).Substring(0, length));
            writer.Write(bytes);
        }
        ...
        private static void WriteId3v2_2TextFrame(BinaryWriter writer, string frameId, string text)
        {
            ...
            int size = textBytes.Length + 1; // +1 for encoding byte
        ...
        private static void WriteId3v2_2CommentFrame(BinaryWriter writer, string comment)
        {
            ...
            byte[] commentBytes = Encoding.GetEncoding("ISO-8859-1").GetBytes(comment);
            
            // Frame ID (COM for v2.2)
            writer.Write(Encoding.ASCII.GetBytes("COM"));
            
            // Frame size (excluding header)
            int size = commentBytes.Length + 5; // +1 for encoding, +3 for language, +1 for empty description
            writer.Write((byte)((size >> 16) & 0xFF));
            writer.Write((byte)((size >> 8) & 0xFF));
            writer.Write((byte)(size & 0xFF));
        ...
        private static void WriteTextFrame(BinaryWriter writer, string frameId, string text)
        {
            ...
            writer.Write(ReverseBytes(BitConverter.GetBytes((UInt32)(textBytes.Length + 1))));
        ...
        {
            // Convert comment to UTF-8
            byte[] textBytes = System.Text.Encoding.UTF8.GetBytes(comment);
            
            // Frame ID (COMM)
            writer.Write(System.Text.Encoding.ASCII.GetBytes("COMM"));
            
            // Frame size (excluding header)
            writer.Write(ReverseBytes(BitConverter.GetBytes((UInt32)(textBytes.Length + 5))));
            ...
            // Language (eng)
            writer.Write(System.Text.Encoding.ASCII.GetBytes("eng"));
