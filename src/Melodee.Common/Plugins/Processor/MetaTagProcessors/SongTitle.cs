...
/// </summary>
public sealed class SongTitle(Dictionary<string, object?> configuration, ISerializer serializer)
    : MetaTagProcessorBase(configuration, serializer)
{
        ...
        FileSystemFileInfo fileSystemFileInfo, MetaTag<object?> metaTag, in IEnumerable<MetaTag<object?>> metaTags)
    {
        ...
        if (songTitle?.Nullify() != null)
        {
            ...
            {
                songNumber =
                    metaTagsValue.FirstOrDefault(x => x.Identifier == MetaTagIdentifier.TrackNumber)?.Value as int? ??
                    songTitle?.TryToGetSongNumberFromString();
                if ((songNumber ?? 0) > 0)
                {
            ...
            if (ContinueProcessing(songTitle))
            {
                if (songTitle.HasFeaturingFragments() || songTitle.HasWithFragments())
                {
                    ...
                    if (songTitle.HasFeaturingFragments())
                    {
                        ...
                        featureArtist = MetaTagsProcessor.ReplaceSongArtistSeparators(StringExtensions
                    ...
                    if (songTitle.HasWithFragments())
                    {
                        ...
                        featureArtist = MetaTagsProcessor.ReplaceSongArtistSeparators(StringExtensions
                            .HasWithFragmentsRegex.Replace(songTitle!.Substring(matches.Index), string.Empty)
                            .CleanString());
                        newSongTitle = SongTitleWithoutWithArtist(songTitle);
                    }

                    featureArtist = featureArtist?.TrimEnd(']', ')').Replace("\"", "'").Replace("; ", "/")
                        .Replace(";", "/");
            ...
            if (ContinueProcessing(songTitle))
            {
                var songTitleRemovals =
                    MelodeeConfiguration.FromSerializedJsonArray(
                        Configuration[SettingRegistry.ProcessingSongTitleRemovals], Serializer);
                if (songTitle != null && songTitleRemovals.Any())
                {
                    songTitle = songTitleRemovals.Aggregate(songTitle,
                        (current, replacement) =>
