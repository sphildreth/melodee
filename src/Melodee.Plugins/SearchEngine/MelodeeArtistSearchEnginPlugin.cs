...
public class MelodeeIArtistSearchEnginPlugin(IMelodeeConfiguration configuration, IDbContextFactory<MelodeeDbContext> contextFactory) : IArtistSearchEnginePlugin
{
        ...
        await using (var scopedContext = await contextFactory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            if (data.Count == 0)
            {
                var artistsByNamedNormalized = await scopedContext.Artists
                    .Select(x => new { x.Id, x.Name, x.NameNormalized, x.ApiKey, x.MusicBrainzId, x.SortName, x.RealName })
